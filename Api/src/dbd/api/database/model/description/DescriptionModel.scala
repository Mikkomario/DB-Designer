...
object DescriptionModel extends FromRowModelFactory[existing.description.Description]
{
	...
	override def apply(model: template.Model[Property]) = table.requirementDeclaration.validate(model).toTry.flatMap { valid =>
		DescriptionRole.forId(valid(descriptionRoleIdAttName).getInt).map { role =>
			existing.description.Description(valid("id").getInt, description.DescriptionData(role, valid("languageId").getInt,
	...
	def insert(data: DescriptionData)(implicit connection: Connection) =
	{
		...
		existing.description.Description(newId, data)
	}
}
...
case class DescriptionModel(id: Option[Int] = None, role: Option[DescriptionRole] = None, languageId: Option[Int] = None,
	...
	extends StorableWithFactory[existing.description.Description]
{
	import DescriptionModel._
	
	// IMPLEMENTED	--------------------------------
	
	override def factory = DescriptionModel
