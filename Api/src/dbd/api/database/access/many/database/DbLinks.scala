 ...
 */
object DbLinks extends ManyModelAccess[database.Link] with NonDeprecatedAccess[database.Link, Vector[database.Link]]
{
	// IMPLEMENTED	----------------------
	...
	class LinksInDatabase(databaseId: Int) extends ChangedModelsAccess[database.Link, LinkConfiguration]
	{
		...
		override def configurationFactory = model.database.LinkConfigurationModel
		
		override def globalCondition = Some(DbLinks.this.mergeCondition(factory.withDatabaseId(databaseId).toCondition))
		
		override def factory = DbLinks.this.factory
		...
		def insert(newConfig: NewLinkConfiguration)(implicit connection: Connection) =
		{
			...
			val insertedConfig = DbLink(newLinkId).configuration.update(newConfig)
	...
	class ClassAttachedLinks(classId: Int) extends ManyModelAccess[database.Link]
	{
		...
		override def globalCondition = Some(DbLinks.this.mergeCondition(
			model.database.LinkConfigurationModel.classConnectColumns.values.map { _ <=> classId }.reduce { _ || _ }))
	...
	class AttributeUsingLinks(attributeId: Int) extends ManyModelAccess[database.Link]
	{
		...
		override def globalCondition = Some(DbLinks.this.mergeCondition(
			model.database.LinkConfigurationModel.withMappingKeyAttributeId(attributeId).toCondition))
