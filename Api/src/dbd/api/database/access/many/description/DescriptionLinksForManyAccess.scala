...
/**
  ...
  * @since 20.5.2020, v2
  */
trait DescriptionLinksForManyAccess[A <: DescriptionLinkLike[Description]] extends DescriptionLinksAccess[A]
{
	// ABSTRACT	------------------------------------
	...
	/**
	  ...
	  */
	protected def subGroup(remainingTargetIds: Set[Int]): DescriptionLinksForManyAccess[A]
	...
	/**
	  ...
	  */
	def inLanguages(languageIds: Seq[Int])(implicit connection: Connection): Map[Int, Vector[A]] =
	{
	...
	protected def inLanguages(remainingTargetIds: Set[Int], languageIds: Seq[Int], remainingRoles: Set[DescriptionRole])(
		implicit connection: Connection): Map[Int, Vector[A]] =
	{
	...
	private def readRemaining(languageIds: Seq[Int],
							  remainingRoles: Set[DescriptionRole], lastAccessPoint: DescriptionLinksForManyAccess[A],
							  lastReadResults: Map[Int, Vector[A]])
							 (implicit connection: Connection): Map[Int, Vector[A]] =
	{
