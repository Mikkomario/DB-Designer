...
/**
  ...
  * @since 3.5.2020, v2
  */
object UserSessions extends ManyModelAccess[existing.UserSession]
{
	// IMPLEMENTED	----------------------------
	...
	class DeviceSessions(deviceId: Int) extends ManyModelAccess[existing.UserSession]
	{
		...
		private def condition = UserSessions.mergeCondition(factory.withDeviceId(deviceId).toCondition)
		
		
		// IMPLEMENTED	-------------------------
		
		override def factory = UserSessions.factory
