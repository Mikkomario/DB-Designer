 ...
 */
object DbClass extends SingleModelAccess[database.Class] with NonDeprecatedAccess[database.Class, Option[database.Class]]
{
	// IMPLEMENTED	-------------------
	 ...
	 */
	class ClassById(classId: Int) extends SingleIdModelAccess[database.Class](classId, DbClass.factory)
	{
		// COMPUTED	-------------------
		
		private def attributeFactory = model.database.AttributeModel
		...
		def markDeleted()(implicit connection: Connection) =
		{
			...
			val classWasDeleted = connection(DbClass.factory.nowDeleted.toUpdateStatement() +
				Where(mergeCondition(DbClass.factory.notDeletedCondition))).updatedRows
			if (classWasDeleted)
			{
				DbLinks.attachedToClassWithId(classId).markDeleted()
				true
			}
			else
				false
		...
		object Info extends SingleModelAccess[ClassInfo]
		{
			...
			override def factory = model.database.ClassInfoModel
		...
		object Attribute extends SingleModelAccess[Attribute]
		{
			...
			class AttributeById(attId: Int) extends SingleIdModelAccess(attId, attributeFactory)
			{
				...
				object Configuration extends SingleModelAccess[AttributeConfiguration]
				{
					...
					override def factory = model.database.AttributeConfigurationModel
		...
		object Attributes extends ManyModelAccess[Attribute]
		{
			...
			def insert(newAttribute: NewAttribute)(implicit connection: Connection) =
			{
				...
				val configurationId = model.database.AttributeConfigurationModel.forInsert(attId, newAttribute.configuration).insert().getInt
