package dbd.api.controller

import java.time.Instant

import dbd.api.database.access.single.database.DbDatabase
import dbd.api.database.model.mysql.{ColumnModel, ReleaseModel, TableModel}
...
import dbd.core.model.existing.mysql.{Release, Table}
import dbd.core.model.partial.mysql
import dbd.core.model.enumeration.NamingConvention._
import dbd.core.model.partial.mysql.{NewColumn, NewColumnAttributeLink, NewColumnLinkLink, NewForeignKey, NewIndex, NewRelease, NewTable}
...
object GenerateTableStructure
{
	...
	def apply(databaseId: Int, newVersionNumber: VersionNumber)(implicit connection: Connection) =
	{
		...
		val classes = DbDatabase(databaseId).classes.all
		// TODO: Handle many-to-many -links separately
		val links = DbDatabase(databaseId).links.all.filterNot { _.linkType.category == ManyToMany }
		...
		val newReleaseId = ReleaseModel.forInsert(newRelease).insert().getInt
		val storedTables = newData.map { table => TableModel.insert(newReleaseId, table) }
	...
	private def linkToColumn(link: Link, linkName: String, namePrefix: String, targetTable: Table) =
	{
		...
		mysql.NewColumn(Left(linkConnection), linkName + "_id", IntType, link.linkType.isOptional)
	}
}
